...
public class IntelligentWoodcutting {
	...
	private WoodcuttingAssignment getClosestAvailableAssignment(Area currentArea, int levelRequired) {
			...
			for (WoodcuttingAssignment currAss : getAvailableAssignments()) {
				if (tempBestAss == null && currAss.getRequiredLevel() == levelRequired) {
					tempBestAss = currAss;
						...
						&& currAss.getWCArea().getRandomPosition().distance(script.myPosition()) < tempBestAss
								.getWCArea().getRandomPosition().distance(script.myPosition())
						&& currAss.getRequiredLevel() == levelRequired) {
					tempBestAss = currAss;
				}
			}
	...
	public WoodcuttingAssignment getSimilarAssignment(Area currentArea, int i) {
		return getClosestAvailableAssignment(currentArea, i);
	}
}
